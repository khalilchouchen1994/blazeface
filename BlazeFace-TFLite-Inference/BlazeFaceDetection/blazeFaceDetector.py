...
class blazeFaceDetector():
	...
	def drawDetections(self, img, results):
		...
		for boundingBox, keypoints, score in zip(boundingBoxes, keypoints, scores):
                        x1 = (self.img_width * max(np.float64(0.0),boundingBox[0])).astype(int)
                        x2 = (self.img_width * max(np.float64(0.0),boundingBox[2])).astype(int)
                        y1 = (self.img_height *max(np.float64(0.0),boundingBox[1])).astype(int)
                        y2 = (self.img_height *max(np.float64(0.0), boundingBox[3])).astype(int)
			...
			#blur face
                        blur = cv2.GaussianBlur(img[y1:y1+h,x1:x1+w],(99,99),cv2.BORDER_DEFAULT)
                        img[y1:y1+h,x1:x1+w] = blur
